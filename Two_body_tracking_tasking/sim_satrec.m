...
%    
%          satrecIR.inclo= iIR;
%          satrecIR.nodeo=OmgIR;% Omg
%          satrecIR.ecco=eIR;
%          satrecIR.argpo=omgIR;
%          satrecIR.mo=MIR;
%          satrecIR.no=sqrt(mu/aIR^3);
% %  -----------------------------------------------------------------------------
% % 
...
% %  ----------------------------------------------------------------------------*/
% 
 function satrec = sim_satrec(satrec,a,e,i,omg,Omg,M)
...
%         satrec.inclo = str2num(longstr2(8:16));%i
%         satrec.nodeo = str2num(longstr2(17:25));%Omg
%         satrec.ecco = str2num(longstr2(26:33));%e
%         satrec.argpo = str2num(longstr2(34:42));%omg
%         satrec.mo = str2num(longstr2(43:51));%M
%         satrec.no = str2num(longstr2(52:63));%mu=n^2*a3
...
%         cardnumb = str2num(longstr2(1));
%         satrec.satnum = str2num(longstr2(3:7));
        satrec.inclo = i;%str2num(longstr2(8:16));
        satrec.nodeo = Omg;%str2num(longstr2(17:25));
        satrec.ecco = e;%str2num(longstr2(26:33));
        satrec.argpo = omg;%str2num(longstr2(34:42));
        satrec.mo = M;%str2num(longstr2(43:51));
        a=a/6380.72609981813;
        satrec.no = 1/(tumin*a^(3/2));%str2num(longstr2(52:63));
%         revnum = str2num(longstr2(64:68));
%     end
...
%     // ---- convert to sgp4 units ----
    satrec.a    = a;%(satrec.no*tumin)^(-2/3);                % [er]
...
%      satrec.EpochDate=[year,mon,day,hr,minute,sec];
...
%                jdstart = satrec.jdsatepoch;%jday( startyear,startmon,startday,starthr,startmin,startsec );
% 
%                stopyear = endtime.year;%input('input stop year');
%                stopmon  = endtime.month;%input('input stop mon');
%                stopday  = endtime.day;%input('input stop day');
%                stophr   = endtime.hr;%input('input stop hr');
%                stopmin  = endtime.min;%input('input stop min');
%                stopsec  = endtime.sec;%input('input stop sec');
...
%                deltamin = dtmin;%input('input time step in minutes ');
%            end;
